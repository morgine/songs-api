...
const (
	// 事件类型消息
	ServerMsgTypeEvent ServerMsgType = "platform"
	// 用户发送文本消息
...
type ServerMessage struct {
	...
	// 消息类型，platform/text/image/voice/video/shortvideo/location/link
...
// MsgType: "platform", Event: "scan" or "subscribe"
// 用户扫描带参数的公众号二维码
...
// MsgType: "platform", Event: "LOCATION"
...
// MsgType: "platform", Event: "TEMPLATESENDJOBFINISH""
// 模板消息发送结果
...
// MsgType: "platform", Event: "MASSSENDJOBFINISH"
// 群发消息结果
...
func Response(serverMsg *ServerMessage, resMsg *ResponseMessage, writer http.ResponseWriter, encrypt *wpt.WXBizMsgCrypt) error {
	resMsg.FromUserName = wpt.Cdata{Value: serverMsg.ToUserName}
	resMsg.ToUserName = wpt.Cdata{Value: serverMsg.FromUserName}
...
func CheckSignature(req *http.Request, token string) (echoStr string, err error) {
	...
	if signature != wpt.SignParams(token, timestamp, nonce) {
...
func ReadServerMessage(req *http.Request, decrypter *wpt.WXBizMsgCrypt) (smsg *ServerMessage, err error) {
	var data []byte
	if decrypter != nil {

...
type ResponseMessage struct {
	...
	ToUserName   wpt.Cdata // 接收方帐号（收到的OpenID）
	FromUserName wpt.Cdata // 开发者微信号(原始ID)
	CreateTime   int64        // 消息创建时间 （整型）

	MsgType ResponseMsgType // text|image|voice|video|music|news

	Content *wpt.Cdata `xml:",omitempty"`
...
type ResArticle struct {
	Title       wpt.Cdata `json:"title"`
	Description wpt.Cdata `json:"description"`
	Url         wpt.Cdata `json:"url"`
	PicUrl      wpt.Cdata `json:"picurl"`
}
...
type ResVideo struct {
	MediaId     wpt.Cdata
	Title       wpt.Cdata `xml:",omitempty"` // 标题(可选)
	Description wpt.Cdata `xml:",omitempty"` // 描述(可选)
}

type ResMusic struct {
	Title        wpt.Cdata `xml:",omitempty"` // 标题(可选)
	Description  wpt.Cdata `xml:",omitempty"` // 描述(可选)
	MusicURL     wpt.Cdata `xml:",omitempty"` // 音乐连接
	HQMusicUrl   wpt.Cdata `xml:",omitempty"` // 高质量音乐链接，WIFI环境优先使用该链接播放音乐
	ThumbMediaId wpt.Cdata `xml:",omitempty"` // 缩略图的媒体id，通过素材管理中的接口上传多媒体文件，得到的id
}
